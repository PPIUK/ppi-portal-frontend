...
function LoginView({ appOAuthLogin }) {
  ...
  const onLoginSubmit = (vals) => {
    auth
      .signin(vals.username, vals.password)
      .then(() => {
        if (
      ...
      .catch((err) => {
        if (err.response.status === 400 || err.response.status === 404)
          return setLoginFeedback(loginErrorAlert("Invalid credentials!"));

        if (err.response.status === 401)
          return setLoginFeedback(
            loginErrorAlert(
              "Email verification pending. Check your Inbox or Spam folder."
            )
          );

        if (err.response.status === 403)
          return setLoginFeedback(loginErrorAlert(err.response.data.message));

        return setLoginFeedback(
          loginErrorAlert("Internal server error. Please try again in a moment")
        );
      });
  };
  ...
  if (auth.user)
    return (
      <Navigate to={query.get("redirect") || "/app/profile/me"}></Navigate>
    );
